...
class XueQiu(object):
    ...
    def Today(self):
        ...
        """
        #取不到链接 只有json 后面考虑加不加入
        #url = xueqiu.com
        #data_url = xueqiu.com/today
        session = requests.session()
        session.get(url, headers=headers)
        resp = session.get(data_url, headers=headers).json()
        for items in resp['items']:
            title = items['original_status']['title']
            description = items['original_status']['description']
            keyword = items['original_status']['text']
